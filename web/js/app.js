(function () {
  ...
  async function startJob(kind) {
    if (state.activeJob) return toast("СНАЧАЛА ЗАВЕРШИ ТЕКУЩУЮ МИССИЮ");
    try {
      const session = await window.api.startJob(kind);
      showActiveJob(session);
    } catch (error) {
      toast(error.message || "НЕ УДАЛОСЬ НАЧАТЬ МИССИЮ");
    }
  }
  ...
  function setWheelAnimation(result) { const chance = Number(result.chance); const end = result.result === "SUCCESS" ? chance * 1.8 : chance * 3.6 + (100 - chance) * 1.8; const wheel = $("#upgrade-wheel"); wheel.style.setProperty("--pointer-end", `${end}deg`); const pointer = $("#upgrade-pointer"); pointer.classList.remove("is-spinning"); void pointer.offsetWidth; pointer.classList.add("is-spinning"); }
