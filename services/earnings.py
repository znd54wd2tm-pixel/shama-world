...
JOB_CONFIG = {
    # kind -> (display kind, time limit, reward, minimum completion time)
    "courier": ("COURIER", 60, 120, 15),
    "factory": ("FACTORY", 30, 90, 0),
    "hunt": ("HUNT", 45, 75, 0),
}
...
def _passive_payload(connection: Any, user: Any, system: str) -> dict[str, Any]:
    ...
    # Existing users from older database versions can have a NULL claim timestamp.
    # Use account creation as the baseline so passive income starts accumulating
    # immediately instead of silently staying at 0 forever.
    last = _parse(user[CLAIM_COLUMNS[system]]) or _parse(user["created_at"])
    now = _now()
...
def start_job(database_path: str, user_id: int, kind: str) -> dict[str, Any]:
    ...
    job_kind, duration, reward, minimum_seconds = config
    with get_connection(database_path) as connection:
        connection.execute("BEGIN IMMEDIATE")
        ...
        return {"success": True, "session_id": int(cursor.lastrowid), "kind": job_kind, "seed": seed,
                "started_at": now.isoformat(), "expires_at": expires.isoformat(), "duration_seconds": duration,
                "minimum_seconds": minimum_seconds, "reward_preview": reward}
...
def complete_job(database_path: str, user_id: int, session_id: int) -> dict[str, Any]:
    with get_connection(database_path) as connection:
        ...
        minimum_seconds = dict((value[0], value[3]) for value in JOB_CONFIG.values()).get(session["kind"], 0)
        if started_at and minimum_seconds > 0 and (now - started_at).total_seconds() < minimum_seconds:
            connection.rollback()
            remaining = max(1, int(minimum_seconds - (now - started_at).total_seconds()))
            raise EarningsError(f"Завершить доставку можно через {remaining} сек.")
