...
The included `render.yaml` defines two Render services from this project root: a web service for FastAPI/static Mini App and a background worker for `python bot.py`. This is necessary because bot polling occupies its process and does not serve the Mini App HTTP URL.

Create the services from the Blueprint and provide `BOT_TOKEN` when Render requests it. `WEBAPP_URL` is configured to the canonical URL in the Blueprint; `bot.py` also uses that same URL directly so an old Render environment value cannot send `/start` to a missing route. `DATABASE_PATH` is set to `shama_world.db`; the API resolves relative database paths from this project directory, not from Render's current working directory. SQLite on Render's default filesystem is ephemeral across redeploys; configure persistent storage before using production player data.

The repository pins Python to 3.12.14 via `.python-version`. The web service command is:

```bash
uvicorn api.routes:app --host 0.0.0.0 --port $PORT
```

The bot worker command is:

```bash
python bot.py
```

The Render Blueprint configures `WEBAPP_URL` exactly as `https://shama-world.onrender.com/`. The bot runs in its own worker process; keep it separate from the Uvicorn web process so Telegram polling owns its event loop without interfering with FastAPI.
