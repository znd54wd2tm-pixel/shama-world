...
from database import (
    ...
    get_upgrade_options, upgrade_item, get_market, market_sell, market_buy, market_cancel, sell_inventory_item, get_collections, claim_pending_drop, get_pending_drops, get_world_missions, start_world_mission, world_mission_step, complete_world_mission, get_npc_market, buy_npc_item
)
...
async def health(request):
    return web.json_response({"ok": True, "version": "19.0"})
...
async def inventory_sell(request):
    try:
        body = await request.json()
    except Exception:
        body = {}
    r = sell_inventory_item(
        request["tg_user"]["id"],
        body.get("item"),
        body.get("quantity", 1)
    )
...
async def pending_drops(request):
    return result({"items": get_pending_drops(request["tg_user"]["id"])})


async def claim_drop(request):
    try: body=await request.json()
    except Exception: body={}
    r=claim_pending_drop(request["tg_user"]["id"], body.get("drop_id"), body.get("action","save"))
...
async def work(request):
    ...
    r = do_work(request["tg_user"]["id"], body.get("job"))
    if not r["ok"]:
        return web.json_response(r, status=409)
    return result(r)
...
async def world_missions(request):
    return result(get_world_missions(request["tg_user"]["id"]))

async def world_mission_start(request):
    try: body=await request.json()
    except Exception: body={}
    r=start_world_mission(request["tg_user"]["id"], body.get("mission"))
    if not r["ok"]: return web.json_response(r,status=409)
    return result(r)

async def world_mission_complete(request):
    try: body=await request.json()
    except Exception: body={}
    r=complete_world_mission(request["tg_user"]["id"], body.get("mission"), body.get("proof"))
    if not r["ok"]: return web.json_response(r,status=409)
    return result(r)

async def world_mission_step_route(request):
    try: body=await request.json()
    except Exception: body={}
    r=world_mission_step(request["tg_user"]["id"],body.get("mission"),body.get("choice"))
    if not r["ok"]: return web.json_response(r,status=409)
    return result(r)

async def npc_market(request):
    return result(get_npc_market(request["tg_user"]["id"]))

async def npc_market_buy(request):
    try: body=await request.json()
    except Exception: body={}
    r=buy_npc_item(request["tg_user"]["id"], body.get("item"), body.get("quantity",1))
    if not r["ok"]: return web.json_response(r,status=409)
    return result(r)
...
def routes(app):
    ...
    app.router.add_post("/api/inventory/sell", inventory_sell)
    app.router.add_get("/api/pending-drops", pending_drops)
    app.router.add_post("/api/drop/claim", claim_drop)
    ...
    app.router.add_get("/api/market/npc", npc_market)
    app.router.add_post("/api/market/npc/buy", npc_market_buy)
    ...
    app.router.add_get("/api/world/missions", world_missions)
    app.router.add_post("/api/world/missions/start", world_mission_start)
    app.router.add_post("/api/world/missions/complete", world_mission_complete)
    app.router.add_post("/api/world/missions/step", world_mission_step_route)
